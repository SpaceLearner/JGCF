...
from .gtn_propogation import GeneralPropagation
...
class GTN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(GTN, self).__init__(config, dataset)
       ...
       #  self.norm_mid_adj_matrix = self.get_norm_mid_adj_mat().to(self.device)
        ...
        self.gp = GeneralPropagation(self.n_layers, config.alpha, cached=True, args=config)
    ...
    def forward(self):
        ...
        lightgcn_all_embeddings, _ = self.gp.forward(all_embeddings, self.norm_adj_matrix)
