...
class SpectralCF(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        A_hat = I + L
        indices = A_hat.coalesce().indices()
        values  = A_hat.coalesce().values()
        self.A_hat = SparseTensor(row=indices[0].to(self.device), col=indices[1].to(self.device), value=values.to(self.device), sparse_sizes=[self.n_users+self.n_items, self.n_users+self.n_items])
    ...
    def forward(self):
        ...
        for k in range(self.n_layers):
            all_embeddings = matmul(self.A_hat, all_embeddings)
