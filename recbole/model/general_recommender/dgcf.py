...
class DGCF(GeneralRecommender):
    ...
    def _build_sparse_tensor(self, indices, values, size):
        ...
        return SparseTensor(row=indices[0], col=indices[1], value=values, sparse_sizes=size).to(self.device)
    ...
    def build_matrix(self, A_values):
        ...
        for i in range(self.n_factors):
            ...
            # (num_edge, 1)
            d_values = matmul(self.edge2head_mat, tp_values)
            ...
            head_term = matmul(self.head2edge_mat, d_values)
            # (num_edge, num_node) (num_node, 1) -> (num_edge, 1)

            tail_term = matmul(self.tail2edge_mat, d_values)
        ...
        for k in range(self.n_layers):
            ...
            for t in range(0, self.n_iterations):
                ...
                for i in range(0, self.n_factors):
                    ...
                    # (num_edge, dim / n_factors)
                    factor_embeddings = matmul(self.edge2head_mat, edge_val)
